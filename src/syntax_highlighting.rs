...
/// which tracks whether we are in a
...
/// use these in conjunction with the `ParseState`
#[derive(Debug)]
enum ParseEvent {
...
impl Default for ParseState {
  fn default() -> ParseState { ParseState::NotInBlock }
}
...
mod tests {
  ...
  fn test_parse_state() {
    ...
    assert_eq!(
      ParseState::NotInBlock.next(&ParseEvent::StartPre(Some(lang.clone()))),
    ...
    assert_eq!(
      ParseState::MaybeStartBlock(lang.clone()).next(&ParseEvent::StartCode),
      ParseState::WillStartCodeBlock(lang.clone())
    );

    assert_eq!(
      ParseState::MaybeStartBlock(lang.clone()).next(&ParseEvent::Text),
      ParseState::NotInBlock
    );

    assert_eq!(
      ParseState::MaybeStartBlock(lang.clone()).next(&ParseEvent::EndCode),
      ParseState::NotInBlock
    );
    ...
    assert_eq!(
      ParseState::MaybeStartBlock(lang.clone()).next(&ParseEvent::Other),
      ParseState::NotInBlock
    );

    assert_eq!(
      ParseState::WillStartCodeBlock(lang.clone()).next(&ParseEvent::Text),
